...
function LogDetailPanel({ logId, onClose }: LogDetailPanelProps) {
  ...
  useEffect(() => {
    ...
    async function loadLog() {
      ...
      try {
        const nextLog = await services.logs.getLogById(logId);
        if (!isActive) {
          return;
        }
              ...
              <PageCard>
                <div className="grid gap-2.5">
                  <div className="rounded-lg bg-surface-subtle/80 p-3">
                    ...
                    <p className={`mt-1 ${valueClassName}`}>{getLogTypeLabel(log.type)}</p>
                  </div>
                  <div className="rounded-lg bg-surface-subtle/80 p-3">
                    <p className={labelClassName}>{t('logs.columns.message')}</p>
