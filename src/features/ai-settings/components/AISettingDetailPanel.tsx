...
function AISettingDetailPanel({
  ...
  useEffect(() => {
    ...
    async function loadSetting() {
      ...
      try {
        const nextSetting = await services.aiSettings.getAISettingById(settingId);
        if (!isActive) {
          return;
        }
  ...
  async function handleSetActive() {
    ...
    try {
      ...
      setActionError(
        error instanceof Error ? error.message : t('aiSettings.actions.activateError'),
      );
    } finally {
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid gap-2.5 sm:grid-cols-2">
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
                      <p className={labelClassName}>{t('aiSettings.form.modelName')}</p>
                      <p className={`mt-1 ${valueClassName}`}>{setting.model_name}</p>
                    </div>
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
                      <p className={labelClassName}>{t('aiSettings.form.temperature')}</p>
                      <p className={`mt-1 ${valueClassName}`}>{setting.temperature.toFixed(2)}</p>
                    </div>
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
