...
function IntegrationConfigDetailPanel({
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      ...
      try {
        const nextConfig = await services.integrations.getIntegrationConfigById(configId);
        if (!isActive) {
          return;
        }
                ...
                <div className="grid gap-3">
                  ...
                  <div className="grid gap-2.5 sm:grid-cols-2">
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
                      <p className={labelClassName}>{t('integrations.configFields.provider')}</p>
                      <p className={`mt-1 ${valueClassName}`}>
                        {getIntegrationProviderLabel(config.provider)}
                      </p>
                    </div>
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
                      <p className={labelClassName}>{t('integrations.configFields.key')}</p>
                      <p className={`mt-1 ${valueClassName}`}>{config.key}</p>
                    </div>
                    <div className="rounded-lg bg-surface-subtle/80 p-3">
                      <p className={labelClassName}>{t('integrations.configFields.isSecret')}</p>
                      <p className={`mt-1 ${valueClassName}`}>
                        {config.is_secret
