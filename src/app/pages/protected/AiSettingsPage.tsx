...
function AiSettingsPage() {
  ...
  async function handleSaveSetting(payload: AISettingMutationInput) {
    ...
    try {
      if (formMode === 'create') {
        await services.aiSettings.createAISetting(payload);
        setCurrentPage(1);
      } else {
        ...
        const updated = await services.aiSettings.updateAISetting(editId, payload);
        if (!updated) {
          throw new Error(t('aiSettings.form.saveError'));
        }
  ...
  async function handleConfirmDelete() {
    ...
    setIsDeleting(true);
    try {
      const deleted = await services.aiSettings.deleteAISetting(settingToDelete.id);
      if (!deleted) {
        throw new Error();
      }
  ...
  async function handleSetActive(id: EntityId): Promise<AISetting | null> {
    const updated = await services.aiSettings.setActiveAISetting(id);
    if (!updated) {
      return null;
    }
