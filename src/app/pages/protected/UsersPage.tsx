...
function UsersPage() {
  ...
  async function handleSaveUser(payload: UserMutationInput) {
    setIsSaving(true);
    setFormErrorMessage(null);
  ...
  async function handleConfirmDelete() {
    ...
    try {
      const deleted = await services.users.deleteUser(userToDelete.id);
      if (!deleted) {
        throw new Error();
      }
  ...
  async function handleToggleUserActive(id: EntityId): Promise<ManagedUser | null> {
        ...
        render: (user) => (
          <span className={tablePrimaryTextClassName}>
            {user.role === 'developer'
              ...
              : t('users.permissionsCount', { count: user.custom_permissions.length })}
          </span>
        ),
      },
      {
